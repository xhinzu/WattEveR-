...
import { Wind, Snowflake, Tv, Disc, Fan, AlertTriangle, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      {/* Overview Card */}
      <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-white/5 shadow-lg relative overflow-hidden">
        {/* Glow */}
        <div className="absolute right-0 top-0 w-24 h-24 bg-cyan-500/10 rounded-full blur-2xl"></div>
        
        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Active Load</h3>
        <div className="mt-1 flex items-baseline space-x-2">
          <span className="text-3xl font-extrabold text-cyan-400 tracking-tight">
            {liveData?.totalWatts || 0}
          </span>
          <span className="text-sm font-semibold text-slate-500">Watts</span>
        </div>
        
        <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-2 gap-4">
          <div>
            <span className="block text-[10px] text-slate-500 uppercase tracking-wider">Monthly Energy</span>
            <span className="text-sm font-bold text-slate-300">{liveData?.monthlyKwh || 0} kWh</span>
          </div>
          <div>
            <span className="block text-[10px] text-slate-500 uppercase tracking-wider">Est. Monthly Bill</span>
            <span className="text-sm font-bold text-teal-400">₹{Math.round((liveData?.monthlyKwh || 0) * 6)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Simulated Appliances</h3>
        <span className="text-[10px] text-cyan-500 bg-cyan-950/30 border border-cyan-800/30 px-2 py-0.5 rounded-full">
          Refreshes live (5s)
        </span>
      </div>
          ...
          return (
            <div 
              ...
              className={`p-4 rounded-xl border transition-all ${
                isOn 
                  ? 'bg-slate-900/50 border-white/10 shadow-md' 
            ...
            >
                ...
                >
                  {isOn ? (
                    <div className="flex items-center space-x-1 text-emerald-400 text-[10px] font-bold">
                      <span>ON</span>
                      <ToggleRight className="w-7 h-7 text-emerald-400 fill-emerald-400/25" />
                    </div>
                  ) : (
              ...
              {isOn && (
                <div className="mt-4 pt-3 border-t border-white/5 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-slate-400">Current Consumption</span>
                    <span className="text-sm font-bold text-slate-200">{watts} W</span>
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-[9px] text-slate-500">
                      ...
                      <span className={isExceeded ? "text-rose-400 font-semibold" : ""}>{usagePercent}%</span>
                    </div>
                  ...
                  {/* Over-limit Warning */}
                  {isExceeded && (
                    <div className="p-2 rounded-lg bg-rose-500/10 border border-rose-500/20 flex items-center space-x-2 text-rose-400 text-[10px] font-medium animate-pulse mt-2">
